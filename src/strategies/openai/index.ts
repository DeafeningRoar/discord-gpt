...
import type { ModelConfig } from '../../services/ai-services/openai';
...
class OpenAIStrategy implements AIStrategy<Response, AICacheStrategy> {
  ...
  async process(
    { id, input, image, txt }: { id: string; name?: string; input: string; image?: string; txt?: string },
  ...
  setSystemPrompt(context?: { source: EVENT_SOURCE; prompt?: string }) {
    ...
    if (context?.source === EVENT_SOURCE.EXPRESS && context?.prompt) {
      this.systemPrompt = context.prompt;
    }
  }
}
